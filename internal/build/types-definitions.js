...
async function addSourceFiles(project) {
  ...
  const filePaths = excludeFiles(
    await glob([globSourceFile, '!icee-ui/**/*'], {
      cwd: pkgRoot, // 读取 packages 目录下除了 icee-ui 目录的文件
  ...
  const epPaths = excludeFiles(
    await glob(globSourceFile, {
      cwd: epRoot, // 读取 ./packages/icee-ui 目录下的文件
      onlyFiles: true, // 只读取文件
    })
  )
  ...
  await Promise.all([
    ...
    ...epPaths.map(async (file) => {
      // 读取 ./packages/icee-ui 目录下的文件，并手动通过 createSourceFile 方法添加 ts-morph 项目的 TypeScript 源文件
